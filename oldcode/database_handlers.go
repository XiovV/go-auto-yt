...
func (db *Database) write() error {
	...
	if err != nil {
		err = fmt.Errorf("orm.write: could not marshal json: %w", err)
		log.Error(err)
		return err
	}
	err = ioutil.WriteFile(filepath.Join(CONFIG_ROOT, db.file), b, 0644)
	if err != nil {
		err = fmt.Errorf("orm.write: could not write file %w", err)
		log.Error(err)
		return err
	}
	return nil
...
func (db *Database) load() error {
	...
	if err != nil {
		err = fmt.Errorf("orm.load: could not read file: %w", err)
		log.Error(err)
		return err
	}
