...
func checkAllTargets(targets []DownloadTarget, targetType string) (Response, error) {
	...
	for _, item := range targets {
		...
		if err != nil {
			return Response{Type: "Error", Key: "GETTING_FROM_DATABASE_ERROR", Message: "There was an error getting the channel from orm" + err.Error()}, fmt.Errorf("CheckAll: %s", err)
		}
